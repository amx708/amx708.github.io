...
<li><b>份额</b>：CR 提升、高端化 → 均价与利润上行，弱化行业周期。</li>
    ...
    'semiconductor-equip': {
        'criteria': '''<p><b>驱动变量</b>：晶圆厂 capex 周期（扩产节奏）、国产替代政策与采购倾斜、先进制程推进（层数 / 结构复杂度提升设备用量）、设备验证与导入节奏、海外设备与零部件限制、下游需求（AI / 存储 / 汽车电子）。</p>
<ul>
...
<li><b>验证节奏</b>：设备从样机到量产导入周期长，验证通过与重复订单是兑现信号。</li>
        ...
        'current': '半导体设备呈"国产替代 + 扩产周期 + 先进制程"三轮驱动：本土 fab 扩产与采购倾斜加速刻蚀 / 薄膜 / 清洗 / CMP / 量测全品类突破，先进制程层数提升推高设备价值量；最大变量是海外限制与下游 capex 周期，验证通过与重复订单是兑现信号。'
    },
}
...
# ============ 数据 ============
from _semiconductor_equip_data import semiconductor_equip
